...
application {
    mainClass.set("ch.vacd.pbll.ApplicationKt")
}

tasks.test {
    useJUnitPlatform()
    // Integration tests hit the live compose stack; skip unless explicitly opted in.
    val includeIntegration = System.getenv("PBLL_INTEGRATION") == "1"
...
tasks.jar {
    manifest { attributes("Main-Class" to "ch.vacd.pbll.ApplicationKt") }
}
