package ch.vacd.pbll.demo

import ch.vacd.pbll.Examples
import ch.vacd.pbll.PrettyJson
import ch.vacd.pbll.clients.EhrbaseClient
import ch.vacd.pbll.ingestion.ImmunizationIngest
...
private fun HTML.renderDemoPage(initialJson: String, initialSlug: String, exampleList: List<Pair<String, String>>) {
    ...
    body {
        ...
        main {
            ...
            div("explain") {
                ...
                ol {
                    ...
                    li { +"PBLL fans Patient/Practitioner/Organization to "; code { +"fhir-server-1" }; +" (HAPI). The FHIR server is the demographics + directory store." }
                    li { +"PBLL ensures an openEHR EHR exists in "; code { +"EHRbase" }; +", linked to the Patient via "; code { +"EHR_STATUS.subject.external_ref" }; +"." }
                    li { +"PBLL calls "; code { +"openFHIR /openfhir/toopenehr" }; +" with the Immunization to get a FLAT openEHR Composition." }
                    li { +"PBLL enriches FLAT with "; code { +"ctx/feeder_audit/original_content" }; +" (verbatim FHIR JSON — Konkretisierung §13)." }
                    li { +"PBLL persists the Composition to EHRbase and returns "; code { +"201 Created" }; +" with "; code { +"compositionUid" }; +"." }
                }
            }
        ...
        footer {
            +"Hackathon harness — services: HAPI 8.8.1 · openFHIR 2.2.1 · EHRbase 2.31.0"
        }
