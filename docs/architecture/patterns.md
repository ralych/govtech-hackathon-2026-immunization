...
|---|---|---|
| **Platform Business Logic Example** | The platform's brain. Receives requests, fans out to storage, exposes helper services, orchestrates conversion. Kotlin + Ktor. | Single place to encode CH VACD-specific routing rules, validation policy, format mediation. Harness smoke-test at `services/platform-business-logic-example/` (not a template for participants). |
...
| `openfhir (2.2.3)) | **Stateless mapping engine** for FHIR ↔ openEHR conversion via FHIRconnect YAMLs. | Lets us declaratively map a CH VACD `Immunization` to an openEHR `Composition` without writing transformation code. Mappings + templates are state; bodies are pure functions. |
| `openfhir-mongo` (Mongo 7.0) | openFHIR's configuration store (templates, contexts, model mappings). | Internal to openFHIR; not addressed by the platform layer directly. |
...
The platform layer is the only service participants build. Everything else is given
infrastructure.

## Pattern A — Authoritative Platform

**Definition.** The platform layer's FHIR endpoint is the system of record. Clients
...
Bundle, the platform layer re-hydrates Patient/Practitioner/Organization from
...
**platform layer responsibilities in Pattern A:**
...
2. **Construction helpers.** `GET /metadata` (own CapabilityStatement),
   ...
   `fhir-server-1` in the current build, so the platform layer either returns 501 or
   serves bundled snapshots from disk.
3. **Validation (deferred).** HAPI on `fhir-server-1` does **not** support
   `$validate` in this build. Client-side profile validation in the platform layer
...
4. **Retrieval** (deferred to a separate PRP).
   ...
   openFHIR does **not** provide; the platform layer implements only the subset of
...
different boundary: the platform layer is an integrator between *external* systems of
...
- **Inbound connectors.** Each upstream system has its own quirks:
  ...
  that only export nightly. Once a resource is inside the platform layer, the
  ingestion path is identical to Pattern A.
- **Outbound connectors.** Consumers either query the platform layer's FHIR endpoint
  (Pattern A read path) or pull bulk extracts via FHIR `$export`. Bulk
  Data Access is its own subprotocol — the platform layer implements it with NDJSON
  output.
- **Sync state metadata.** Watermarks per source, idempotency keys per
  ...
  next to the platform layer (NOT in EHRbase or `fhir-server-1`).
- **Conflict resolution hooks.** Policy for "two upstream sources recorded
  the same vaccination differently". The platform layer needs a hook (last-writer-
...
from clinic systems, or any deployment where the platform layer is operationally
...
|---|---|---|
| Where data originates | Direct client POST to platform layer | External FHIR systems |
| Where data is served from | platform layer's FHIR endpoint | platform layer's FHIR endpoint (and/or external consumers) |
| Number of runtime services to demo | 5 (platform layer + fhir-server-1 + openFHIR + Mongo + EHRbase + EHRbase-db) | 5–7 (same + optional `fhir-server-2`, sync-state Postgres) |
| Number of FHIR endpoints clients see | 1 (platform layer) | 1–N (platform layer + external sources/sinks the platform layer doesn't control) |
...
- **Identity provider.** Citizen authentication (SwissID? SMART on FHIR?)
  is a separate layer in front of the platform layer. Out of scope for the harness;
  flagged here so it's not forgotten.
- **Terminology server.** Konkretisierung (2026-05-12) pins to
  `tx.fhir.ch/r4` (FHIRsmith-based). platform layer value-set lookups and code
...
- **Reverse proxy / API gateway.** Konkretisierung shows a gateway
  ...
  platform layer. Not in the harness; participants who want one bring their own.
